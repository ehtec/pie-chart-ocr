...
def pie_chart_generator(labels, data, legend=True):
...
def generate_random_pie_chart(num_sectors, legend=True):
    ...
    pie_chart_generator(labels=all_label_list, data=areas, legend=legend)
...
def main():
    ...
    for counter in range(TOTAL_CHARTS_COUNT):
        ...
        generate_random_pie_chart(random.randint(MIN_SECTOR_COUNT, MAX_SECTOR_COUNT), legend=True)
